...
-export([exit/1, get_cpu/0, get_os/0, download_file/1, download_zip/1,
         extract_sqlc_binary/1, extract_sqlc_from_targz/1, os_command/5, os_exit/1, os_which/1,
         start_arguments/0]).
...
os_command(Command, Args, Dir, Opts, EnvBin) ->
  ...
  {ExitCode, Output} =
    case Which of
      ...
      {ok, Executable} ->
        ...
        FromBin =
          fun({Name, Val}) ->
             {binary_to_list(Name), unicode:characters_to_list(Val, file:native_name_encoding())}
          end,
        Env = lists:map(FromBin, EnvBin),

        PortSettings =
          lists:merge([[{args, Args}, {cd, Dir}, {env, Env}, eof, exit_status, hide, in],
        ...
        try
          ...
          case LetBeStdout of
            true ->
              {Status, <<>>};
            _ ->
              {Status, list_to_binary(OutputChars)}
          end
        catch
          Error:Reason ->
  ...
  case ExitCode of
    ...
    2 when Output == <<>> ->
      DirError =
        list_to_binary("The directory \"" ++ binary_to_list(Dir) ++ "\" does not exist\n"),
  ...
  Result =
    case PathType of
      ...
      relative ->
        ...
        AbsDir =
          case filename:pathtype(DirStr) of
            absolute ->
              DirStr;
            relative ->
              filename:absname(DirStr)
          end,
